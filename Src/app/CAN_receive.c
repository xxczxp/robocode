...
//-hk-读取can接收数据，根据帧ID，分别放于motor_chassis数组或rm_imu_data结构体中
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    CAN_RxHeaderTypeDef rx_header;
    uint8_t rx_data[8];
		//-hk-从CAN_RX_FIFO0中读取帧头存于rx_header中、数据存于rx_data数组中；
...
//CAN发送0x700的ID数据，会引发M3508进入快速设置ID模式
