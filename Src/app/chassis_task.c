...
static void chassis_mode_change_control_transit(chassis_move_t *chassis_move_transit)
{
	...
	//切入跟随底盘角度模式
	if ((chassis_move_transit->last_chassis_mode != CHASSIS_VECTOR_SPEED) && chassis_move_transit->chassis_mode == CHASSIS_VECTOR_SPEED)
	{
		chassis_move_transit->chassis_yaw_set = chassis_move_transit->chassis_yaw;
	}
	//切入不跟随云台模式
	else if ((chassis_move_transit->last_chassis_mode != CHASSIS_VECTOR_RAW) && chassis_move_transit->chassis_mode == CHASSIS_VECTOR_RAW)
	{
		chassis_move_transit->chassis_yaw_set = chassis_move_transit->chassis_yaw;
	}
...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
    ...
    chassis_move_update->chassis_gyro_x = *(chassis_move_update->chassis_INS_gyro + INS_GYRO_X_ADDRESS_OFFSET);
    chassis_move_update->chassis_gyro_y = *(chassis_move_update->chassis_INS_gyro + INS_GYRO_Y_ADDRESS_OFFSET);
    chassis_move_update->chassis_gyro_z = *(chassis_move_update->chassis_INS_gyro + INS_GYRO_Z_ADDRESS_OFFSET);
}

//遥控器的数据处理成底盘的前进vx速度，vy速度
//-hk-根据遥控器手柄位，计算得到底盘的vx_set、vy_set值
...
//设置遥控器输入控制量
//-hk-将底盘的vx_set、vy_set写入结构体中，以便chassis_control_loop调用
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
...
//-hk-通过遥控器设定的底盘vx_set、vy_set速度值，计算得到4个麦轮的设定电流
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    //根据上面得到的麦轮设定值，计算轮子控制最大速度，并限制其最大速度
    for (i = 0; i < 4; i++)
    {
