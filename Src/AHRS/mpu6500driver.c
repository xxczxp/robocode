...
static const uint8_t write_mpu6500_reg_data_error[MPU6500_Write_Reg_Num][3] =
    {
        ...
        {MPU_INTBP_CFG,
         ((~MPU_INTBP_OPEN) & (~MPU_LATCH_INT_EN) & (~MPU_INT_ANYRD_2CLEAR) & (~MPU_ACTL_FSYNC) & (~MPU_FSYNC_INT_MODE_EN)) & (MPU_INTBP_ACTL | MPU_BYPASS_EN),
         INTBP_CFG_ERROR},
...
void mpu6500_read_over(uint8_t *status_buf, mpu6500_real_data_t *mpu6500_real_data)
{
    ...
    else if (mpu6500_real_data->status & (1 << MPU_MOT_BIT))
    {
...
void gyro_offset_calc(fp32 gyro_offset[3], fp32 gyro[3], uint8_t imu_status, uint16_t *offset_time_count)
{
